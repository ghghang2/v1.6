...
1. Sent **from your own address** (`ghghang2@gmail.com`).
2. Subject contains `nbchat` (routes to the assistant) **or** `supervisor`
   (routes to the supervisor, when one is running — see §4).
3. Sent **since this chat session started** (a 60 s grace window). Older
...
`supervisor` is routed to the supervisor instead of the assistant, and the
...
Supervisor emails are given **queue priority** over normal emails, so a
question is answered in real-time even if it arrives while a long normal
email turn is still queued.
...
| `send_email` | Send a plain-text Gmail email. | `send_email(subject="…", body="…")` |
