...
class ChatUI:
    ...
    def _stream_assistant_response(self, client, tools, chat, messages):
        ...
        # Use stream_and_collect from chat module
        assistant_text, tool_calls, finished, reasoning_text = chat.stream_and_collect(client, messages, tools)
        ...
        assistant_placeholder.value = f'<div style="background-color: #f1f8e9; padding: 10px; border-radius: 10px; margin: 5px;"><b>Assistant:</b> {assistant_text}</div>'
        ...
        if tool_calls and not finished:
            # Process tool calls using the chat module
            self.history = chat.process_tool_calls(
